        ...
        <>
                                ...
                                <TableRow key={user.id}>
                                    ...
                                    <TableCell>
                                        <Button variant="contained" size="small" sx={{mr: 1}}
                                                onClick={() => handleViewDetailsClick(user)} disabled={detailsLoading}>View
                                            Details</Button>
                                        <Button variant="outlined" size="small" sx={{mr: 1}}
                                                onClick={() => handleEditClick(user)} disabled={detailsLoading}>Edit
                                            Username</Button>
                                        <Button variant="outlined" size="small" onClick={() => handleResetClick(user)}
                                                disabled={detailsLoading}>Edit Password</Button>
                                    </TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
            ...
            <Dialog open={showDetailsModal} onClose={handleDetailsClose} maxWidth="md" fullWidth>
                ...
                <DialogContent dividers>
                    {detailsLoading &&
                        <Box sx={{textAlign: 'center'}}><CircularProgress size="sm"/> <Typography variant="body2">Loading
                            ...
                            <Card sx={{mb: 3}}>
                                ...
                                <List dense>
                                    ...
                                    <ListItem><ListItemText
                                        primary={`Email: ${selectedUserDetail.user.email}`}/></ListItem>
                                    <ListItem><ListItemText
                                        primary={`Role: ${selectedUserDetail.user.role}`}/></ListItem>
                                </List>
                            </Card>
                                    ...
                                    {selectedUserDetail.posts.length > 0 ? (
                                        <List dense>
                                            {selectedUserDetail.posts.map(post => (
                                                <ListItem key={post.id} component={RouterLink} to={`/board/${post.id}`}
                                                          onClick={handleDetailsClose}>
                                                    <ListItemText
                                                        primary={`${post.title} ${post.secret ? '(비밀글)' : ''}`}/>
                                                </ListItem>
                                            ))}
                                        </List>
                                    ) : (
                                    ...
                                    {selectedUserDetail.comments.length > 0 ? (
                                        <List dense>
                                            {selectedUserDetail.comments.map(comment => (
                                                <ListItem key={comment.id} component={RouterLink}
                                                          to={`/board/${comment.postId}`} onClick={handleDetailsClose}>
                                                    <ListItemText
