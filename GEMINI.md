...
1. **보안 원리 파악 (Learning by Principle):** 보안의 근본 원리를 깊이 체득하기 위해, 외부 라이브러리(Spring Security 등)의 자동화된 기능에 의존하지 않고 자바와 스프링의 순수 기능(`HttpSession`, `Filter` 등)을 사용하여 직접 구현하며 그 원리를 상세히 설명한다.
2. **정확한 보안 패치 (Accurate Patching):** 휴먼 에러를 방지하고 확실한 보안 적용을 위해 AI가 직접 소스 코드를 수정하며, 수정 전/후(Before/After) 비교를 통해 사용자의 학습을 돕는다. 
